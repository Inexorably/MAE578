...
const char SPHERE_COLOR_A = 'a';
const char SPHERE_COLOR_B = 'b';
...
void SphereColorToggle() {
    if (sphere_color_current == SPHERE_COLOR_A) {
        for (int i = 0; i < 4; ++i){
			sphere_color[i] = sphere_color_b[i];
			sphere_color_current = SPHERE_COLOR_B;
		}
    }
    else {  //...sphere_color == sphere_color_b
        for (int i = 0; i < 4; ++i){
			sphere_color[i] = sphere_color_a[i];
			sphere_color_current = SPHERE_COLOR_A;
		}
    }
}
...
void displayFunction(void)
{
	...
	////////////////////////////////////////////
	//NOTE: IF WE WANT TO TURN OFF THE PROXYING OF THE LARGER SPHERE ON THE MIDDLE WALL, DELETE THIS IF STATEMENT.
	if (proxy_object[0]+sphere_radius > -0.5*center_wall_thickness)	{
		proxy_object[0] = -center_wall_thickness/2 - sphere_radius;
	}
	////////////////////////////////////////////
	...
	//Draw the attractive field.
	GLUquadricObj* aQuadObj = gluNewQuadric();
    drawSphere(aQuadObj, attractive_sphere_pos, attractive_sphere_color, attractive_sphere_radius);
    gluDeleteQuadric(aQuadObj);
...
HDCallbackCode HDCALLBACK DynamicObjectsCallback(void* data)
{
    ...
    if (sphere_f.magnitude() > 0 && !sphere_collision) {
        SphereColorToggle();
